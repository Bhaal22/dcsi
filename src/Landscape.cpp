 ...
 * 					   int nbSubDiv : ras
 */
Landscape::Landscape(int jpeg, char *fileTxrJpg, char *txr, float dim, int subdiv1, int f, float fMult, float fAdd, GLfloat red, GLfloat green, GLfloat blue) : 
										dimension(dim), nbSubdiv(subdiv1), factor(f), factorMult(fMult), factorAdd(fAdd), r(red), g(green), b(blue)
{
		/* Load the JpegFile */
		if (jpeg == JPEG)
		{
...
float Landscape::getYFromXZ(float x, float z)
{
  ...
  P1[0] = -ext + i*pas;  P1[1] = elevation(i,      j) * factorMult + factorAdd; P1[2] = -ext + j*pas;
  P2[0] = -ext + (i+1)*pas;  P2[1] = elevation(i+1,  j) * factorMult + factorAdd; P2[2] = -ext + j*pas;
  P3[0] = -ext + (i+1)*pas;   P3[1] = elevation(i+1,  j+1) * factorMult + factorAdd; P3[2] = -ext + (j+1)*pas;
  P4[0] = -ext + i*pas;  P4[1] = elevation(i,      j+1) * factorMult + factorAdd;     P4[2] = -ext + (j+1)*pas;       
...
void Landscape::shape()
{
  ...
  for (i=0; i< nbSubdiv ; i++)
    for (j=0; j<nbSubdiv ; j++) 
    {
      ...
      /* Triangle 1 */
      glTexCoord2f((j+1)*pas/pas2, i*pas/pas2);            glVertex3fv(P4);
      glTexCoord2f((j+1)*pas/pas2, (i+1)*pas/pas2);     glVertex3fv(P3);
      glTexCoord2f(j * pas/pas2, i*pas/pas2);                 glVertex3fv(P1);

      /* Triangle 2 */
      glTexCoord2f((j+1)*pas/pas2, (i+1)*pas/pas2);      glVertex3fv(P3);
      glTexCoord2f(j * pas/pas2, (i+1)*pas/pas2);           glVertex3fv(P2);
      glTexCoord2f(j * pas/pas2, i*pas/pas2);                  glVertex3fv(P1);
      glEnd();
    }
