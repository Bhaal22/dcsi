...
/* Dessine une box texturÅÈe avec de l'alpha */
  ...
  for(i=0;i<100;i++)
    {
	 ...
	 pvr_wait_ready();
       ...
       if(cpt<75) drawFont("Press Start !",30,180,350,0.4,0.7,0.93);
       pvr_list_finish();
       pvr_scene_finish();
       cpt=(cpt+1)%100;
       thd_sleep(20);
...
void Menu2D::draw_gen_font(int choixtexte)
{
  ...
  if(choixtexte==0)
  ...
  else if(choixtexte==2)
    {
      ...
      drawFont("\"Demain matin Å‡ 15h... C est bon ?\"",18,80,305,1.0,1.0,1.0);
    }
...
void Menu2D::draw_perso(void)
{
    ...
    if(poscadre==0) drawFont("Carlos",25,270,320,0.0,0.0,0.0);
    else if(poscadre==1) drawFont("Momo",25,275,320,0.0,0.0,0.0);
    else if(poscadre==2) drawFont("Noel",25,275,320,0.0,0.0,0.0);
    pvr_list_finish();
    pvr_scene_finish();
...
/* Affichage des caractÅÈristiques des diffÅÈrents persos*/
...
/* Affichage Å‡ l'ÅÈcraznde la VMU */
void Menu2D::vmu_lcd_update(int choixim) {
  if(choixim==0) vmu_set_icon(tex_blanc);
  else if(choixim==1) vmu_set_icon(tex_game);
  else if(choixim==2) vmu_set_icon(tex_carlos);
  else if(choixim==3) vmu_set_icon(tex_1);
  else if(choixim==4) vmu_set_icon(tex_2);
  else if(choixim==5) vmu_set_icon(tex_3);
  else if(choixim==6) vmu_set_icon(tex_go);
}

/*Affichage du compte Å‡ rebour*/
...
/*Gestion de l'ÅÈcriture sur la VMU*/
...
/* VÅÈrifie la prÅÈsence de fichiers de sauvegarde*/
...
/*Affiche les diffÅÈrents record sauver sur la VMU Å‡ l'ÅÈcran */
void Menu2D::draw_record()
{
  ...
  for(i=0;j!=2;i++)
    {
      if(i<3) nom1[i]=mesrecords[i];
      else{
      if(mesrecords[i]==' ') j++;
      if(mesrecords[i]=='.') k++;
      mot1[i-3]=mesrecords[i];
      if(k==2&&mesrecords[i]=='.')mot1[i-3]='\"';
      if(k==3&&mesrecords[i]=='.')mot1[i-3]='\"';
      }
    } 
  mot1[10]='\0';
  j=0;
  ...
  for(l=0;j!=2;l++)
    {
      if(l<3) nom2[l]=mesrecords[l+i];
      else{
	if(mesrecords[l+i]==' ') j++;
	if(mesrecords[l+i]=='.') k++;
	mot2[l-3]=mesrecords[i+l];
	if(k==2&&mesrecords[i+l]=='.')mot2[l-3]='\"';
	if(k==3&&mesrecords[i+l]=='.')mot2[l-3]='\"';
      }
    } 
  mot2[10]='\0';
  j=0;
  ...
  for(m=0;j!=2;m++)
    {
      if(m<3) nom3[m]=mesrecords[m+l+i];
      else{
	if(mesrecords[m+l+i]==' '||mesrecords[m+l+i]=='\0') j++;
	if(mesrecords[m+l+i]=='.') k++;
	mot3[m-3]=mesrecords[i+l+m];
	if(k==2&&mesrecords[i+l+m]=='.')mot3[m-3]='\"';
	if(k==3&&mesrecords[i+l+m]=='.')mot3[m-3]='\"';
      }
    } 
  mot3[10]='\0';
  ...
  while(!done) {
    ...
    pvr_list_begin(PVR_LIST_TR_POLY);
    draw_alp_tex_box(0.0,0.0,1024.0,1024.0,0.1,1024,lesTextures[fondgris],1.0);
    if(!strcmp(nom1,"mom")) draw_alp_tex_box(180.0,130.0,244.0,194.0,1.0,256,lesTextures[momo],1.0);
    else if(!strcmp(nom1,"noe")) draw_alp_tex_box(180.0,130.0,244.0,194.0,1.0,256,lesTextures[noel],1.0);
    else if(!strcmp(nom1,"car")) draw_alp_tex_box(180.0,130.0,244.0,194.0,1.0,256,lesTextures[carlos],1.0);
    
    if(!strcmp(nom2,"mom")) draw_alp_tex_box(180.0,170.0,244.0,234.0,1.0,256,lesTextures[momo],1.0);
    else if(!strcmp(nom2,"noe")) draw_alp_tex_box(180.0,170.0,244.0,234.0,1.0,256,lesTextures[noel],1.0);
    else if(!strcmp(nom2,"car")) draw_alp_tex_box(180.0,170.0,244.0,234.0,1.0,256,lesTextures[carlos],1.0);
    
    if(!strcmp(nom3,"mom")) draw_alp_tex_box(180.0,210.0,244.0,274.0,1.0,256,lesTextures[momo],1.0);
    else if(!strcmp(nom3,"noe")) draw_alp_tex_box(180.0,210.0,244.0,274.0,1.0,256,lesTextures[noel],1.0);
    else if(!strcmp(nom3,"car")) draw_alp_tex_box(180.0,210.0,244.0,274.0,1.0,256,lesTextures[carlos],1.0);
