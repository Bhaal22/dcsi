...
public:
  ...
  /* divide a quad in 2 triangles */
  virtual void addQuad(float x1, float y1, float z1,
		       float x2, float y2, float z2,
		       float x3, float y3, float z3,
		       float x4, float y4, float z4) = 0;
  virtual void addQuad(float v1[3],float v2[3],float v3[3],float v4[4]) = 0;
...
class TimeoutExpired {
  public :
    static void error () {printf("*** WARNING *** !!! *** Time out expired error (coldet)\n");}
};
...
class Inconsistency {
  public :
    static void error () {printf("*** WARNING *** !!! *** Inconstency error (coldet)\n");}
};
