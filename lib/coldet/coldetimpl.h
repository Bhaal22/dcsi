...
public:
  ...
  void addQuad(float x1, float y1, float z1,
    ...
    addQuad(Vector3D(x1,y1,z1),
	    Vector3D(x2,y2,z2),
	    Vector3D(x3,y3,z3),
	    Vector3D(x4,y4,z4));
  }
  void addQuad(float v1[3],float v2[3],float v3[3],float v4[4]) {
    addQuad(Vector3D(v1[0],v1[1],v1[2]),
	    Vector3D(v2[0],v2[1],v2[2]),
	    Vector3D(v3[0],v3[1],v3[2]),
	    Vector3D(v4[0],v4[1],v4[2]));
  }
  //arrayVecT1 and 2 are 2 array of 3 points (Vector3D) each where to store the newly created triangles
  void addQuad(const Vector3D& v1, const Vector3D& v2, const Vector3D& v3, const Vector3D& v4, Vector3D* arrayVecT1 = NULL, Vector3D* arrayVecT2 = NULL);
